...
<body class="bg-gray-100" x-data="{ itemModal: false, modalContent: '' }">
            ...
            <form action="{{ route('items.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                ...
                <div class="w-full md:w-48">
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="under_repair" {{ request('status') == 'under_repair' ? 'selected' : '' }}>Under
                            Repair</option>
                            ...
                            <tr class="item-row" onclick="viewItem({{ $item->id }})">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        @else
                                            <div
                                                class="flex-shrink-0 h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center mr-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        @if ($item->isCurrentlyLoaned()) bg-yellow-100 text-yellow-800
                                        @else
    ...
    <!-- Item Details Modal -->
    <div x-show="itemModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <!-- Modal Backdrop -->
        <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity" @click="itemModal = false"></div>

        <!-- Modal Content -->
        <div class="flex items-center justify-center min-h-screen">
            <div class="relative bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto"
                @click.outside="itemModal = false" x-transition:enter="transition ease-out duration-300"
                ...
                <!-- Close Button -->
                <button @click="itemModal = false" class="absolute top-4 right-4 text-gray-400 hover:text-gray-500">
